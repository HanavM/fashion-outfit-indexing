...
# "long" removed entirely -- weights renormalize automatically since
# random.choices doesn't require them to sum to 1.
LABEL_KIND_WEIGHTS = {
    "generic": 0.10,
    "attribute": 0.22,
    "brand": 0.18,
...
def build_training_labels(product):
    ...
    for facet in ("color", "material", "fit", "pattern"):
...
def encode_images(model, image_records):
    ...
    for start in range(0, len(image_records), IMAGE_EVAL_BATCH_SIZE):
...
def evaluate_exact_retrieval_category_scoped(model, evaluation_records, candidate_labels, category_of_exact_label):
    ...
    for category, image_positions in records_by_category.items():
        ...
        image_position_tensor = torch.tensor(image_positions, dtype=torch.long)
        cat_image_embeddings = image_embeddings[image_position_tensor]

        target_local_indices = torch.tensor([
            local_index_of_global[candidate_to_index[valid_records[p]["exact_label"]]]
            for p in image_positions
        ], dtype=torch.long)

        similarities = cat_image_embeddings @ cat_text_embeddings.T
        target_scores = similarities[torch.arange(len(image_positions)), target_local_indices]
        ranks = (similarities > target_scores[:, None]).sum(dim=1) + 1
        all_ranks.append(ranks.float())
        scored_records += len(image_positions)
...
def run_stage(model, mode, epochs, lr, save_dir, stage_name):
    ...
    for epoch in range(1, epochs + 1):
        ...
        with (OUTPUT_DIR / f"{stage_name}_training_history.json").open("w", encoding="utf-8") as f:
        ...
        if selection_score > best_score:
            ...
            with (save_dir / "validation_metrics.json").open("w", encoding="utf-8") as f:
                json.dump({
...
# ============================================================
# RESUMED RUN: stage 1 (heads-only, 6/8 epochs) already completed and
# saved to Drive in a previous session that was interrupted by a Colab
# idle-timeout disconnect. Skip stage 1 entirely and continue straight
# into stage 2 from that checkpoint -- avoids re-paying ~6 epochs of GPU
# + Drive I/O time for a result we already have.
# ============================================================

import json as _json
with (STAGE1_DIR / "validation_metrics.json").open("r", encoding="utf-8") as f:
    _stage1_saved = _json.load(f)

stage1_best = _stage1_saved["selection_score"]
print(f"Resuming from saved stage1_heads checkpoint: {STAGE1_DIR}")
print(f"Stage 1 best val R@1 (global): {_stage1_saved['exact_metrics']['recall_at_1'] * 100:.2f}%")
print(f"Stage 1 best val R@1 (category-scoped): {_stage1_saved['exact_metrics_category_scoped']['recall_at_1'] * 100:.2f}%")

baseline_exact = _stage1_saved["exact_metrics"]
baseline_exact_scoped = _stage1_saved["exact_metrics_category_scoped"]
...
model = AutoModel.from_pretrained(STAGE1_DIR, torch_dtype=torch.float32).to(DEVICE)
stage2_best = run_stage(model, "last_block", STAGE2_EPOCHS, STAGE2_LR, STAGE2_DIR, "stage2_last_block")
