...
v3 test result (category-scoped): R@1 18.83%, R@5 51.09%, R@10 67.59%,
MRR 34.12%, median rank 5.0. A follow-up per-label-kind breakdown (run
against this checkpoint) showed that number is really the "model" kind's
score in disguise -- generic (category) hit 95.09% R@1, brand 61.39%,
while attribute-family kinds dragged the aggregate down: material 50.08%,
color 31.57%, closure 23.45%, fit 20.05%, defining_features 15.57%.

Modal body variant of finetune_siglip2_v4.py -- only DATASET_ROOT differs
(/data/apparel_dataset, the Modal Volume mount point), same convention as
finetune_siglip2_v3_modal_body.py.

v4 change (one, targeted): v2/v3 pooled every structured facet (color/
material/fit/pattern/closure/silhouette/length) plus defining_features and
the free-text caption-fragment "attribute" bucket into a single "attribute"
kind, and sample_training_label draws *uniformly across whichever entries
a product has* within a chosen kind. That means each facet's actual
training frequency was an accident of how many entries it happens to carry
per product, not a deliberate choice. Measured: defining_features averages
3.61 entries/product vs. color's 1.52 and material's 1.92 -- so color's
text targets were being crowded out during training by the more talkative
defining_features field, a plausible real contributor to color's mediocre
31.57% (vs. material's 50.08%) despite color being the more visually
salient, "classically easy" CLIP-style cue. fit (20.05%) and closure
(23.45%) were weak independent of this -- both have small candidate pools
(106 and 62), ruling out candidate-space size as the explanation, more
likely the model just hasn't learned those construction-level visual
distinctions ("regular" vs. "relaxed" fit, "zip" vs. "button" closure)
as well as it has surface-appearance ones.

v4 gives every facet (color/material/fit/pattern/closure/silhouette/
length/defining_features/attribute_caption) its own top-level kind and
explicit LABEL_KIND_WEIGHTS entry instead of one shared pool -- see that
constant's own comment for the specific weights and rationale. Nothing
else changes from v3 (same hard-negative mining, InfoNCE option, layer-
wise unfreezing, K=4 batch, multi-text sampling) -- this is a single,
isolated, testable lever so its effect on the by-facet breakdown is
attributable, not tangled up with other simultaneous changes.

Two things NOT addressed by this v4 change, flagged rather than guessed
at: (1) a real data-quality issue behind color's raw values -- 390 unique
color strings scraped verbatim from brand pages, many of them marketing-
name fragmentation of the same visual color no vision model could
distinguish from a photo (e.g. "sail"/"cloud white"/"summit white"/"cream
white"/"core white"/"off white" as six variants of near-identical
whites) -- would need a canonicalization pass analogous to
build_hierarchy.py's category canonicalization, scoped separately since it
needs careful manual/LLM-assisted judgment, not a training-loop change.
(2) fit/closure's weakness could also warrant construction-specific hard-
negative mining (same category, different fit/closure value) rather than
just more sampling weight -- not implemented here, worth trying if the
reweighting alone doesn't move those two enough.
"""
...
OUTPUT_DIR = DATASET_ROOT / "finetuned_siglip2_hierarchical_v4"
...
# v4 change: v2/v3 pooled every structured facet (color/material/fit/
# pattern/closure/silhouette/length) plus defining_features into one
# shared "attribute" kind, and sample_training_label draws *uniformly
# across whichever entries a product has* within a chosen kind -- so a
# product's relative facet weighting was accidentally proportional to how
# many entries each facet happens to have, not to how much that facet
# actually needs training signal. Measured real effect from the v3
# checkpoint's per-facet eval: defining_features averages 3.61 entries/
# product vs. color's 1.52 and material's 1.92, so color's text targets
# were getting crowded out during training -- plausible contributor to
# color's mediocre 31.57% R@1 (vs. material's 50.08%) despite color being
# the more "classically easy" CLIP-style visual cue. fit (20.05%) and
# closure (23.45%) were weak too, independent of this dilution issue (both
# have small candidate pools, ruling out candidate-space size as the
# explanation -- more likely the model hasn't learned those specific
# construction-level visual distinctions well).
#
# Fix: give every facet its own explicit top-level kind and weight
# instead of one shared pool, so sampling frequency is a deliberate
# choice, not an artifact of how chatty the LLM captioner was for a given
# facet on a given product. color/fit/closure boosted directly; brand/
# generic trimmed slightly since both are already near-saturated (95%/61%
# R@1 in the v3 breakdown); model trimmed since a smaller total attribute-
# family share needed to go somewhere and model is already the single
# largest bucket. First-pass weights, not exhaustively tuned -- rerun the
# by-facet eval after this and adjust from there.
LABEL_KIND_WEIGHTS = {
    "generic": 0.08,
    "brand": 0.16,
    "model": 0.30,
    ...
    "pattern": 0.01,
    "silhouette": 0.01,
    "defining_features": 0.10,
    "attribute_caption": 0.07,
}
...
def build_training_labels(product):
    ...
    # v4 change: each facet gets its own kind (was collapsed into one
    # "attribute" kind in v2/v3) so LABEL_KIND_WEIGHTS controls each
    # facet's training frequency directly instead of it being an accident
    # of how many entries a product happens to have for that facet (see
    # LABEL_KIND_WEIGHTS module note above for the measured evidence this
    # was actually hurting color specifically).
    ...
    # products (1075/1234) and the richest per-product visual-grounding
    # signal in the schema (e.g. "premium suede upper", "elasticized waist
    # with inner drawcords", "triple Gazelle branding"). Highest entries/
    # product of any facet (3.61 avg) -- exactly why it needed its own
    # kind rather than pooling with the low-entry-count facets above.
    ...
    for position, text_item in enumerate(positive_texts):
        if text_count <= 1:
            ...
            if fraction < 0.2:
                ...
                # Free-text LLM caption fragment, not a specific
                # structured facet -- its own kind for the same reason
                # the structured facets got split out above.
