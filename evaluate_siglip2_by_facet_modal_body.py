...
the "attribute" kind scores only 27.20% R@1. That number lumps every
attribute facet -- color, material, fit, pattern, closure, silhouette,
length, plus free-text defining_features -- into one flat ~2,633-candidate
pool per image, so a query whose correct answer is "black" gets ranked
against "regular fit," "leather," "cropped," etc. all at once. This script
tests whether that's a measurement artifact hiding real per-facet strength
(e.g. color might be excellent, material near-chance) by evaluating each
facet as its own separate candidate pool, exactly like generic/brand/model
already are in the other script.
...
output_path = DATASET_ROOT / "finetuned_siglip2_hierarchical_v3" / "eval_by_facet.json"
