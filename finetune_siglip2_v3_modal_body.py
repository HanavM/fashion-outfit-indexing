...
Modal body variant of finetune_siglip2_v3.py -- only DATASET_ROOT differs
(/data/apparel_dataset, the Modal Volume mount point, instead of the Colab
Drive mount point). Run via `modal run modal_app_v3.py`, not directly.
Kept as a separate file rather than a runtime branch on an env var because
that's the existing convention (finetune_siglip2_modal_body.py did the
same for v2) and it keeps a Modal run's exact executed source pinned and
diffable against the Colab version.
